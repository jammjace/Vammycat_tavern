...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: 'test-results/art-preview.png' });
    await page.evaluate(()=>{const s=game.scene.getScene('GameScene');s.cameras.main.stopFollow();s.cameras.main.centerOn(1900,580);});
    await page.waitForTimeout(100);await page.screenshot({path:'test-results/art-house-preview.png'});
    await page.evaluate(()=>{const s=game.scene.getScene('GameScene');s.scene.resume();});
    await page.mouse.move(660,400);await page.mouse.wheel(0,500);await page.waitForTimeout(450);
    const scrolled=await page.evaluate(()=>{const s=game.scene.getScene('GameScene');s.scene.pause();return {hour:s.sunSystem.hour,clock:s.sunSystem.uiText.text};});
    ...
    const town=await page.evaluate(()=>{
      const s=game.scene.getScene('GameScene'),t=s.level.treePositions[0];
    ...
    const effects=await page.evaluate(()=>{
      const s=game.scene.getScene('GameScene'),p=s.player,w=s.level.waterZones[0];
    ...
    await page.evaluate(()=>game.scene.getScene('GameScene').scene.resume());
    ...
    const zoom=await page.evaluate(()=>{const s=game.scene.getScene('GameScene');s.scene.pause();return {world:s.cameras.main.zoom,ui:s.uiCamera.zoom};});
    ...
    await page.evaluate(()=>{const s=game.scene.getScene('GameScene');s.cameras.main.stopFollow();s.cameras.main.centerOn(1200,800);});
    await page.waitForTimeout(100);await page.screenshot({path:'test-results/town-preview.png'});
